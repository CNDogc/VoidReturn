...
> 版本 / Version 2.0.5 · Paper 1.17.x – 26.x（理论兼容；仅 26.2 实测）· 轻量来源记忆型虚空传送插件
...
1. 将 `VoidReturn-2.0.5.jar` 放入服务器 `plugins/` 目录。Put the jar into the server's `plugins/` folder.
...
- 兼容 / Compatibility：Paper 1.17.x – 26.x（单个 jar 通吃；api-version `1.17`，Java 17 字节码，26.x 服以 Java 25 运行）。**1.17 – 1.21 为理论兼容（未逐一实测），仅 26.2 实测。** One jar for Paper 1.17.x – 26.x (api-version `1.17`, Java 17 bytecode; 26.x servers run it on Java 25). **1.17–1.21 is theoretical (not individually tested); only 26.2 is verified.**
...
- 构建 / Build：`.\gradlew.bat build`，产物 / artifact：`build/libs/VoidReturn-2.0.5.jar`
